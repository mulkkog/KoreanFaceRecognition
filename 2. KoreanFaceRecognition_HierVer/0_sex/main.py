...
#InceptionResnet = InceptionResnetV1(pretrained='casia-webface', device='cuda:0', classify=True, num_classes=2)
...
test_loader = torch.utils.data.DataLoader(
    Dataset(test_csv_path, test_image_path), batch_size=batch_size, shuffle=False, num_workers=8, pin_memory=True
)
train_loader = torch.utils.data.DataLoader(
    Dataset(train_csv_path, train_image_path), batch_size=batch_size, shuffle=True, num_workers=8, pin_memory=True
)
...
for i in range(epoch):
    ...
    if cur_acc > best_acc:
        torch.save(Resnet, './checkpoint/sex_model_2021_01_18_resnet50.pt')
        best_acc = cur_acc
        print('saved')
writer.close()
